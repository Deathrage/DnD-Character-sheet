...
describe('createCharacter', () => {
  it('produces a document that passes the v2 schema', () => {
    const result = characterDocumentV2Schema.safeParse(createCharacter(INPUT));
  ...
  describe('does not share substructure between documents', () => {
    ...
    const cases: Array<{
      name: string;
      mutate: (doc: CharacterDocumentV2) => void;
      read: (doc: CharacterDocumentV2) => unknown;
      original: unknown;
    }> = [
      {
      ...
      {
        name: 'spellList.spellcasting',
        mutate: (doc) => {
          doc.spellList.spellcasting.intelligence = { attackBonus: 5, saveDc: 13 };
        },
        read: (doc) => doc.spellList.spellcasting,
        original: {},
      },
      {
