...
function validDocument() {
  return {
    ...
    equipment: {
      weapons: [
        ...
        },
        {
          ...
          description: 'No attack roll entered yet.',
          attuned: false,
          equipped: false,
          attack: null as {
            ability: (typeof ABILITY_KEYS)[number];
    ...
    spellList: {
      ...
      spellcasting: {
        intelligence: { attackBonus: 7, saveDc: 15 },
        wisdom: { attackBonus: -1, saveDc: 9 },
      } as Partial<Record<(typeof ABILITY_KEYS)[number], { attackBonus: number; saveDc: number }>>,
    },
    counters: {
      categories: [
        {
...
describe('characterDocumentV2Schema', () => {
  it('accepts a complete valid document', () => {
    expect(characterDocumentV2Schema.safeParse(validDocument()).success).toBe(true);
  });

  it('requires schemaVersion to be exactly 2', () => {
    const doc = { ...validDocument(), schemaVersion: 1 };
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });
  ...
  it('accepts two classes sharing a name, which the business layer rejects rather than the schema', () => {
    ...
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(true);
  });
  ...
  it('requires all six abilities and all eighteen skills', () => {
    ...
    expect(characterDocumentV2Schema.safeParse(withoutAbility).success).toBe(false);

    const withoutSkill = validDocument();
    delete (withoutSkill.abilitiesAndSkills.skills as Record<string, unknown>).survival;
    expect(characterDocumentV2Schema.safeParse(withoutSkill).success).toBe(false);
  });
  ...
  it('requires an attack key on every weapon: "no attack" is null, never a missing key', () => {
    const doc = validDocument();
    delete (doc.equipment.weapons[1] as Record<string, unknown>).attack;
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it('accepts every ability as a weapon attack ability', () => {
    for (const ability of ABILITY_KEYS) {
      const doc = validDocument();
      doc.equipment.weapons[0]!.attack = { ability, attackBonus: 5, damage: '' };
      expect(characterDocumentV2Schema.safeParse(doc).success).toBe(true);
    }
  });

  it.each(['STR', 'luck', '', null])('rejects %j as a weapon attack ability', (ability) => {
    const doc = validDocument();
    doc.equipment.weapons[0]!.attack = { ability, attackBonus: 5, damage: '' } as never;
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it('accepts a negative attack bonus and rejects a fractional one', () => {
    const negative = validDocument();
    negative.equipment.weapons[0]!.attack = { ability: 'strength', attackBonus: -2, damage: '' };
    expect(characterDocumentV2Schema.safeParse(negative).success).toBe(true);

    const fractional = validDocument();
    fractional.equipment.weapons[0]!.attack = { ability: 'strength', attackBonus: 1.5, damage: '' };
    expect(characterDocumentV2Schema.safeParse(fractional).success).toBe(false);
  });

  it('rejects a weapon attack with no ability, because the ability is what gates it', () => {
    const doc = validDocument();
    doc.equipment.weapons[0]!.attack = { attackBonus: 5, damage: '' } as never;
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it.each([' 1d8', 'x'.repeat(81)])('rejects %j as damage', (damage) => {
    const doc = validDocument();
    doc.equipment.weapons[0]!.attack = { ability: 'strength', attackBonus: 5, damage };
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it('does not accept an attack on other equipment — only weapons carry one', () => {
    const doc = validDocument();
    (doc.equipment.other[0] as Record<string, unknown>).attack = null;
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it('accepts an empty spellcasting record, and requires the key', () => {
    const empty = validDocument();
    empty.spellList.spellcasting = {};
    expect(characterDocumentV2Schema.safeParse(empty).success).toBe(true);

    const missing = validDocument();
    delete (missing.spellList as Record<string, unknown>).spellcasting;
    expect(characterDocumentV2Schema.safeParse(missing).success).toBe(false);
  });

  it('keys spellcasting by ability name only', () => {
    const doc = validDocument();
    (doc.spellList.spellcasting as Record<string, unknown>).luck = { attackBonus: 1, saveDc: 9 };
    expect(characterDocumentV2Schema.safeParse(doc).success).toBe(false);
  });

  it('rejects a negative spell save DC but accepts a negative spell attack bonus', () => {
    const negativeDc = validDocument();
    negativeDc.spellList.spellcasting.wisdom!.saveDc = -1;
    expect(characterDocumentV2Schema.safeParse(negativeDc).success).toBe(false);

    const negativeAttack = validDocument();
    negativeAttack.spellList.spellcasting.wisdom!.attackBonus = -3;
    expect(characterDocumentV2Schema.safeParse(negativeAttack).success).toBe(true);
  });

  it('rejects a negative armour class', () => {
    expect(
      characterDocumentV2Schema.safeParse({ ...validDocument(), armorClass: -1 }).success,
    ).toBe(false);
  });
  ...
  it('reports the duplicate at the second occurrence, not the first', () => {
    ...
    const result = characterDocumentV2Schema.safeParse(doc);
...
const unknownKeyLocations: Array<[string, (doc: Doc) => void]> = [
  ...
  // In v2 weapons and other equipment are separate schema instances, so each needs its own row.
  [
    'an other-equipment item',
    (doc) => {
      (doc.equipment.other[0]! as Record<string, unknown>).extra = 'x';
    },
  ],
  [
    'a weapon attack',
    (doc) => {
      (doc.equipment.weapons[0]!.attack as Record<string, unknown>).extra = 'x';
    },
  ],
  // The key is wrong, not the value: the value is a valid entry, so only the record's key set
  // can reject it.
  [
    'the spellcasting record',
    (doc) => {
      (doc.spellList.spellcasting as Record<string, unknown>).extra = {
        attackBonus: 1,
        saveDc: 9,
      };
    },
  ...
  [
    'a spellcasting entry',
    (doc) => {
      (doc.spellList.spellcasting.intelligence as Record<string, unknown>).extra = 'x';
    },
  ],
  [
    'the spell list section object',
    (doc) => {
      (doc.spellList as Record<string, unknown>).extra = 'x';
    },
  ],
  [
    'a spell entry',
