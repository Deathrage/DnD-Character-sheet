...
describe('characterDocumentV3Schema', () => {
  it('accepts a complete valid document', () => {
    expect(characterDocumentV3Schema.safeParse(validDocument()).success).toBe(true);
  });

  it('requires schemaVersion to be exactly 3', () => {
  ...
  it('accepts two classes sharing a name, which the business layer rejects rather than the schema', () => {
    ...
    expect(characterDocumentV3Schema.safeParse(doc).success).toBe(true);
  });
  ...
  it('requires all six abilities and all eighteen skills', () => {
    ...
    expect(characterDocumentV3Schema.safeParse(withoutAbility).success).toBe(false);

    const withoutSkill = validDocument();
    delete (withoutSkill.abilitiesAndSkills.skills as Record<string, unknown>).survival;
    expect(characterDocumentV3Schema.safeParse(withoutSkill).success).toBe(false);
  });
  ...
  it('accepts an empty spellcasting record, and requires the key', () => {
    ...
    expect(characterDocumentV3Schema.safeParse(empty).success).toBe(true);

    const missing = validDocument();
    delete (missing.spellList as Record<string, unknown>).spellcasting;
    expect(characterDocumentV3Schema.safeParse(missing).success).toBe(false);
  });
  ...
  it('rejects a negative spell save DC but accepts a negative spell attack bonus', () => {
    ...
    expect(characterDocumentV3Schema.safeParse(negativeDc).success).toBe(false);

    const negativeAttack = validDocument();
    negativeAttack.spellList.spellcasting.wisdom!.attackBonus = -3;
    expect(characterDocumentV3Schema.safeParse(negativeAttack).success).toBe(true);
  });
  ...
  it('rejects a negative armour class', () => {
    expect(
      characterDocumentV3Schema.safeParse({ ...validDocument(), armorClass: -1 }).success,
    ).toBe(false);
  });
  ...
  it('reports the duplicate at the second occurrence, not the first', () => {
    ...
    const result = characterDocumentV3Schema.safeParse(doc);
